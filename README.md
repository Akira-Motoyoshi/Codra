...
- 現在募集中の選考情報：募集状況、締切、選考ステップ、応募・公式リンクを表示
...
企業カードと企業詳細の「選考情報」タブでは、企業側の募集状況を以下の区分で表示します。

- `募集中`：サンプルの募集情報があり、締切が過ぎていない状態
- `受付終了`：締切を過ぎた募集情報
- `募集開始前`：将来の募集を想定した状態
- `情報確認中`：募集情報がない、または公式確認が必要な状態

あわせて、選考区分、応募締切、次回締切、選考ステップ、応募ページ、募集要項、企業公式採用ページ、情報ソース、確認日を表示します。7日以内の締切は注意色で表示します。リンク先はデモ用サンプルで、実際の応募には使えません。

企業情報・選考情報は2026年7月16日時点のサンプルです。古い可能性があるため、応募前に必ず企業公式サイトで最新情報を確認してください。

企業の募集状況と、ユーザー自身の選考状況は別の情報です。前者は「募集中」「受付終了」など企業側の状態、後者は`codra_apps`に保存された「応募予定」「ES提出済み」「面接」などの自分の進捗を表します。両者を混同しないよう、ダッシュボードでも「企業募集」と「あなたの選考」を分けて表示します。
...
├── demo.css                # デモUI
├── recruitment.css         # 募集状況・選考情報UI
├── recruitment.js          # 募集データと表示ロジック
├── navigation.js            # モバイルメニュー制御
